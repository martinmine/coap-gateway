...
public final class HttpTranslator {
     ...
     * HttpHeader, ContentType, CustomMediaTypeRegistry
     */
    public static int getCoapMediaType(HttpMessage httpMessage) {
        ...
        int coapContentType = CustomMediaTypeRegistry.UNDEFINED;
        ...
        if (contentType != null) {
            ...
            if (coapContentTypeString != null) {
                ...
                // mapping
                coapContentType = CustomMediaTypeRegistry.parse(httpContentTypeString);
            }
        }
            ...
            try {
                ...
                if (optionNumber == CustomOptionNumberRegistry.ACCEPT) {
                    ...
                    for (String headerFragment : headerValue.split(",")) {
                        ...
                        Integer[] coapContentTypes = {CustomMediaTypeRegistry.UNDEFINED};
                        if (headerFragment.contains("*")) {
                            coapContentTypes = CustomMediaTypeRegistry.parseWildcard(headerFragment);
                        } else {
                            coapContentTypes[0] = CustomMediaTypeRegistry.parse(headerFragment);
                        }
        ...
        if (payload != null && payload.length != 0) {
            ...
            if (!coapMessage.getOptions().hasContentFormat()) {
                ...
                if (coapContentTypeString == null || coapContentTypeString.isEmpty()) {
                    coapContentTypeString = CustomMediaTypeRegistry.toString(coapContentType);

                    // if the coap content-type is printable, it is needed to
                    // set the default charset (i.e., UTF-8)
                    if (CustomMediaTypeRegistry.isPrintable(coapContentType)) {
     ...
     *
     * @param coapRequest the coap request
    ...
    public static void getHttpResponse(HttpRequest httpRequest, Response coapResponse, HttpResponse httpResponse) throws TranslationException {
        ...
        if (!httpRequest.getRequestLine().getMethod().equalsIgnoreCase("head")) {
            ...
            // text-plain
            if (coapResponse.getOptions().getContentFormat() == CustomMediaTypeRegistry.UNDEFINED
                    && (ResponseCode.isClientError(coapCode)
                    || ResponseCode.isServerError(coapCode))) {
                LOGGER.info("Set contenttype to TEXT_PLAIN");
                coapResponse.getOptions().setContentFormat(CustomMediaTypeRegistry.TEXT_PLAIN);
            }
